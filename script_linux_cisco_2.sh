...
# I/O redirection allows users to divert the standard destination or source of a command's data stream to or from files, devices, or other commands. Instead of using text editors, redirection provides a powerful mechanism to write, overwrite, or append command outputs directly into text files within the filesystem.

# To redirect the standard output (STDOUT) of a command to a file, overwriting its existing content:
COMMAND > FILE

# To redirect the standard output (STDOUT) of a command to a file, appending to its existing content:
COMMAND >> FILE

# --- The Three Linux Standard File Descriptors ---
# 1. STDIN  (Standard Input):  Data stream provided to a command, typically received from the keyboard (Descriptor 0).
# 2. STDOUT (Standard Output): Data stream generated by a successful command, typically displayed on the monitor (Descriptor 1).
# 3. STDERR (Standard Error):  Error messages generated by a failed or invalid command execution (Descriptor 2).

# To change the current working location to the Documents directory:
cd ~/Documents

# To display the structural content of a specific file on the screen:
cat food.txt

# To divert the screen output of cat and save it into a new or existing file (overwriting any previous data):
cat food.txt > newfile1.txt

# To print a literal string phrase or message back onto the terminal display:
echo "Hello"

# To overwrite the entire content of a file with a specific custom text string:
echo "I like food." > newfile1.txt

# To insert or add a new text string line at the bottom of a file without destroying its current contents:
echo "This food is good." >> newfile1.txt

# --- Summary of Redirecciones ---
# - Single Greater-Than (>)  : Overwrites the target file's content entirely. If the file does not exist, it creates it.

# - Double Greater-Than (>>) : Appends the new output line directly to the end of the target file, preserving its existing data.
